...
class data_generation:
    def __init__(
        ...
        self.ensemble_weights = np.zeros(
            (self.num_neuron * self.num_ensemble, self.num_ensemble)
        )
        for i in range(self.num_ensemble):
            self.ensemble_weights[
                i * self.num_neuron : (i + 1) * self.num_neuron, i
            ] = 1
    ...
    def generate_z(self):
        ...
        if self.kernel_type == "exp":
            dist = distance.cdist(
                np.linspace(1, self.len_data, self.len_data).reshape(
                    (self.len_data, 1)
                ),
                np.linspace(1, self.len_data, self.len_data).reshape(
                    (self.len_data, 1)
        ...
        if self.periodicity:
            if self.bump_placement == "random":
                rf_location = (
                    2
                    * np.pi
                    * np.random.rand(self.num_neuron * self.num_ensemble, self.dim)
                )
            ...
            if self.bump_placement == "random":
                rf_location = np.random.rand(
                    self.num_neuron * self.num_ensemble, self.dim
                )
                ...
                rf_location = np.tile(
                    np.array(
                        [
                            min_z
                            + [
                                (i + 0.5) / self.num_neuron * (max_z - min_z)
                                for i in range(self.num_neuron)
                            ]
                            for j in range(self.dim)
                        ]
    ...
    def generate_spikes(self, z, rf_location):
        ...
        selector = np.concatenate(
            np.split(selector, self.num_ensemble, axis=1), axis=3
        ).reshape(
        ...
        response = np.log(self.back_firing) + (
            np.log(self.peak_firing / self.back_firing)
        ) * np.exp(-dist / (2 * self.tuning_width))
...
def get_data(
    num_neuron_train, num_neuron_test, len_data_train, len_data_test, index, global_seed
):
