...
class Trainer:
    ...
    def train(self):
        ...
        for i in range(self.num_steps + 1):
            ...
            y_train = self.data_train[self.neurons_train_ind][
                :, ind : ind + self.batch_size
            ]
            y_test = self.data_train[self.neurons_test_ind][
                :, ind : ind + self.batch_size
            ]
            if self.mode != "full":
            ...
            if self.model.latent_style == "hack":
                ...
                kld_loss = torch.distributions.kl.kl_divergence(
                    q_z, p_z
                ).mean()  # mean over time and latents
                poisson_loss = (
                    torch.nn.PoissonNLLLoss(log_input=False, reduction="none")(
                        y_train_, y_train
                    ).mean()
                    + torch.nn.PoissonNLLLoss(  # mean over neurons and time
                        log_input=False, reduction="none"
                    )(
                        y_test_, y_test
                    ).mean()  # mean over neurons and time
                ) / 2
            ...
            if i > 0 and not (i % self.num_log_step):
                ...
                if self.model.latent_style == "hack":
                    y_train_, y_test_, z_, mu, logvar = self.model(
                        y_train, z=self.z_test
                    )
                    if self.model.normalize_encodings:  # if normalized, take out of KL.
                        kld_loss = compute_kld_to_normal(
                            torch.zeros_like(logvar), logvar
                        )
                    else:
                        kld_loss = compute_kld_to_normal(mu, logvar)
                    ...
                    y_train_, y_test_, z_, mu, logvar, q_z, p_z = self.model(
                        y_train, z=self.z_test
                    )
                    kld_loss = torch.distributions.kl.kl_divergence(q_z, p_z).mean()
                    poisson_loss_train = torch.nn.PoissonNLLLoss(
                        log_input=False, reduction="none"
                    )(
                        y_train_, y_train
                    ).mean()  # mean over neurons and time
                    poisson_loss_test = torch.nn.PoissonNLLLoss(
                        log_input=False, reduction="none"
                    )(
                        y_test_, y_test
                    ).mean()  # mean over neurons and time
                ...
                slowness_loss = compute_slowness_loss(mu)
                if self.mode != "full":
                    encoder_loss = torch.sum((angle2vector(z) - angle2vector(z_)) ** 2)
                    print("encoder_loss=", encoder_loss, file=self.log_file)
                corrs = []
                for j in range(y_test.shape[0]):
                    corrs.append(
                        pearsonr(
                            y_test[j].detach().cpu().numpy(),
        ...
        for j in range(y_test.shape[0]):
            corrs.append(
                pearsonr(
                    y_test[j].detach().cpu().numpy(), y_test_[j].detach().cpu().numpy()
                )[0]
            )
