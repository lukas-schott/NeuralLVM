...
def analysis(ensembler, model, trainer, z_test, do_inference=False):
    ...
    if ensembler.latent_style == "hack":
        ...
        _, y_, z_, mu, logvar, _, _ = ensembler(
    ...
    ensemble_weights = (
        torch.nn.functional.softmax(ensembler.ensemble_weights_test, dim=1)
        .detach()
        .cpu()
        .numpy()
    )
    ...
    for i in range(latent_dim * num_ensemble):
        ...
        plt.plot(
            trainer.data_test[trainer.neurons_test_ind][i * 25, :200]
            .detach()
            .cpu()
            .numpy()
        )
    ...
    learned_rfs = (
        ensembler.receptive_fields_test.detach()
        .cpu()
        .numpy()
        .reshape(true_rfs.shape[0], -1)
    )
